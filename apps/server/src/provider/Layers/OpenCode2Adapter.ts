...
const resolveNotificationTurnId = (ctx: OpenCode2SessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: OpenCode2SessionContext): TurnId | undefined =>
  ctx.activeTurnId;
...
export function parseOpenCode2Resume(raw: unknown): { sessionId: string } | undefined {
  if (!isRecord(raw)) return undefined;
  if (raw.schemaVersion !== OpenCODE2_RESUME_VERSION) return undefined;
...
export function opencode2PromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process OpenCode2 ACP callback.",
              cause,
            }),
        ),
      );
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = opencode2PromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native OpenCode2 notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const requireSession = (
      threadId: ThreadId,
    ): Effect.Effect<OpenCode2SessionContext, ProviderAdapterSessionNotFoundError> => {
      const ctx = sessions.get(threadId);
      if (!ctx || ctx.stopped) {
        return Effect.fail(
    ...
    const startSession: OpenCode2AdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const requestedStartModelId = opencode2ModelSelection?.model
            ? resolveOpenCode2AcpBaseModelId(opencode2ModelSelection.model)
            : undefined;
          const boundModelId = yield* applyOpenCode2AcpModelSelection({
            runtime: acp,
            model: requestedStartModelId,
            selections: opencode2ModelSelection?.options,
            mapError: (cause): ProviderAdapterError =>
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveOpenCode2AcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process OpenCode2 runtime notification.", { cause }),
            ),
            Effect.forkChild,
          );
          ...
          ctx.exitFiber = yield* ctx.acp.waitForExit.pipe(
            Effect.flatMap(() =>
              withThreadLock(
                ctx.threadId,
                Effect.gen(function* () {
    ...
    const sendTurn: OpenCode2AdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolveOpenCode2AcpBaseModelId(turnModelSelection.model)
                : undefined;
              const currentModelId = yield* applyOpenCode2AcpModelSelection({
                runtime: ctx.acp,
              ...
              const displayModel = currentModelId
                ? resolveOpenCode2AcpBaseModelId(currentModelId)
                : undefined;
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "OpenCode2 prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "OpenCode2 prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "OpenCode2 session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "OpenCode2 session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "OpenCode2 prompt request failed.",
                }),
              );
    ...
    const interruptTurn: OpenCode2AdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
    ...
    const respondToRequest: OpenCode2AdapterShape["respondToRequest"] = (
      threadId,
      requestId,
      decision,
    ) =>
    ...
    const respondToUserInput: OpenCode2AdapterShape["respondToUserInput"] = (
      threadId,
      requestId,
      answers,
    ) =>
    ...
    const readThread: OpenCode2AdapterShape["readThread"] = (threadId) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
        return { threadId, turns: ctx.turns };
      });

    const rollbackThread: OpenCode2AdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "OpenCode2 ACP sessions do not support provider-side rollback yet.",
        });
      });

    const stopSession: OpenCode2AdapterShape["stopSession"] = (threadId) =>
      withThreadLock(
        threadId,
        Effect.gen(function* () {
          const ctx = yield* requireSession(threadId);
